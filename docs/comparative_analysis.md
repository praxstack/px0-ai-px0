...
- **Dry-Run Mode for Workflow Verification**: Allows testing a workflow structure, verifying inputs, and previewing planned actions before calling real external APIs or writing changes. px0 has no blanket rehearsal mode; a run always fires for real. It scores `~` through a narrower, standing mechanism instead: a workflow's `confirm:` (or the store-wide `tools.confirm_writes`) holds specific write calls back and shows exactly what would be sent, for a person to approve or reject (`px0 approvals`), which covers "before calling real external APIs" without a separate flag to remember.
